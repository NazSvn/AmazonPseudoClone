...
class Product {
  ...
  getStarsUrl() {
    ...
    return `assets/ratings/rating-${averageRating * 10}.png`;
  }
  getPrice() {
    return `${formatCurrency(this.priceCents)}`;
  }
  getAverageRating() {
    return getRatingAverage(this.rating.userRating);
  }
}

const getRatingAverage = (ratings) => {
  const minRatings = 10;
  const globalAverage = 0.0;
  const totalRatings = ratings.length || 10;
  const sumOfRatings = ratings.reduce((acc, rating) => acc + rating, 0);

  const finalRating =
    (globalAverage * minRatings + sumOfRatings) / (minRatings + totalRatings);

  return roundToHalf(finalRating);
};
...
const defaultProducts = [
  ...
  {
    id: 'bc2847e9-5323-403f-b7cf-57fde044b958',
    image: 'assets/products/umbrella.jpg',
    name: 'Umbrella',
    rating: {
      percentage: {},
      userRating: [],
...
export const loadProducts = () => {
  ...
  if (savedProducts) {
    products = JSON.parse(savedProducts).map(
